...
MOOD:    dict = _load(MOOD_FILE)      # {chat_id: {mood:"", date:""}}
WISHES:  dict = _load(WISHES_FILE)    # {chat_id: {name: [wishes...]}}
...
# pending_wish: {chat_id: birthday_name} — ждём пожелания от пользователя
pending_wish:      dict = {}
...
# Ответы Эльзы на триггеры в зависимости от настроения
MOOD_ELZA_REPLIES = {
    "злая": [
...
# Дефолтные ответы (когда нет настроения или для режимов без переопределения)
MAT_REPLIES_DEFAULT = [
    "ой всё, следи за базаром 💅",
...
# Ответы обидчику при повторном появлении
OFFENDER_REPLIES = [
...
# Напоминания об обратном отсчёте (за 7, 3, 1 день)
COUNTDOWN_PHRASES = {
...
async def cmd_groups(message: types.Message):
    ...
    if not DATA:
        ...
        return
    lines = ["📋 <b>Группы где используется бот:</b>\n"]
    for i, (cid, group) in enumerate(DATA.items(), 1):
        count = len(group)
        lines.append(f"{i}. chat_id: <code>{cid}</code> — {count} чел. в списке ДР")
    await message.answer("\n".join(lines))
...
async def handle_text(message: types.Message):
    ...
    # Логируем всё что пишут — пересылаем админу
    if user_id != ADMIN_ID:  # себя не логируем
        await log_message(message)

    # Записываем активность (для любимчика недели)
    ...
    if state is None and mentions_elza(text):
        # Сначала проверяем — обидчик ли это?
    ...
    if state is None:
        ...
        # Обидчик написал — иногда напоминаем
...
async def reminder_loop():
    ...
    reminded:         set = set()
    daily_said:       set = set()
    countdown_sent:   dict = {}   # {chat_id:name:days} -> date
    poll_sent:        dict = {}   # {chat_id:name} -> date
        ...
        try:
                    ...
                    for name, info in group.items():
                        ...
                        if d == 7:
                            ...
                            if poll_sent.get(poll_key) != day_key:
                                ...
                                options = list(dict.fromkeys(options))[:10]  # убираем дубли, макс 10
                                try:
                                    await bot.send_poll(
                                        int(chat_id),
            ...
            if now.weekday() == 6 and now.hour == 20 and now.minute == 0 and week_key not in fav_announced:
                ...
                for chat_id in list(DATA.keys()):
                    ...
                    # Сбрасываем счётчик активности на новую неделю
